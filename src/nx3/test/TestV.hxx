...
class TestV extends  haxe.unit.TestCase 
{
	...
	public function testVNotePause() 
	{
		var vnote = new VNote(new NNote(ENoteType.Pause(0)));
		this.assertEquals(0, vnote.nnote.nheads.length);		
		this.assertEquals(ENoteValue.Nv4, vnote.nnote.value);
		
		var vheads = vnote.getVHeads();
		this.assertEquals(vheads.length, 0);
	}	
	...
	public function testVNoteInternalDirection()
	{
		...
		// pause§
		var calculator = new VNoteInternalDirectionCalculator(new VNote(new NNote(ENoteType.Pause(0))).getVHeads());
		this.assertEquals(EDirectionUD.Down, calculator.getDirection());
	...
	public function testVHeadsPlacementsCalculator() 
	{
		...
		var calculator = new VHeadPlacementCalculator(vnote.getVHeads(), EDirectionUD.Down);
		var placements = calculator.getHeadsPlacements();		
		this.assertEquals(placements.length, 0);
	...
	public function testVNoteDirectionCalculator() 
	{
		...
		//---------------------------------------------------------------------
		// always down for non-notes...
		var vnote = new VNote(new NNote(ENoteType.Pause(0), EDirectionUAD.Up));
		var calculator = new VNoteDirectionCalculator(vnote);
		this.assertEquals(EDirectionUD.Down, calculator.getDirection(EDirectionUD.Up));
