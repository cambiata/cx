...
class RenderBase 
{
	...
	*/	
	...
	private function _drawTpl(x:Float, y:Float, dnote:DNote) {
		...
		*/
		...
		gr.drawCircle(cx, cy, radius);
		gr.endFill();
#else
		var shape:Shape = SvgAssets.getSvgShape('tplCircle', this.scaling);				
		shape.x = tplX;
		shape.y = tplY;		
		target.addChild(shape);
#end
		// number
		var shape = SvgAssets.getSvgShape(tpl, this.scaling);		
		shape.x = tplX;
		shape.y = tplY;		
		target.addChild(shape);
		...
		switch (dnote.note.heads.first().sign) {
			case ESign.Sharp:
				var shape = SvgAssets.getSvgShape('tplArrowUp', this.scaling);		
				shape.x = tplX;
				shape.y = tplY;		
				target.addChild(shape);
			case ESign.Flat:
				var shape = SvgAssets.getSvgShape('tplArrowDown', this.scaling);		
				shape.x = tplX;
				shape.y = tplY;		
				target.addChild(shape);
			default:	
