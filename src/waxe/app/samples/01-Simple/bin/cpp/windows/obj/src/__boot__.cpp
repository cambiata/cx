#include <hxcpp.h>

#include <wx/TextCtrl.h>
#include <wx/StaticText.h>
#include <IntHash.h>
#include <haxe/Resource.h>
#include <Type.h>
#include <ValueType.h>
#include <wx/Bitmap.h>
#include <ApplicationMain.h>
#include <Simple.h>
#include <Std.h>
#include <wx/DC.h>
#include <wx/Pen.h>
#include <haxe/io/Error.h>
#include <wx/MenuBar.h>
#include <wx/Button.h>
#include <IntIter.h>
#include <haxe/io/Bytes.h>
#include <wx/Panel.h>
#include <wx/EventID.h>
#include <wx/Menu.h>
#include <wx/Error.h>
#include <wx/Frame.h>
#include <wx/TopLevelWindow.h>
#include <wx/FlexGridSizer.h>
#include <wx/GridSizer.h>
#include <wx/Font.h>
#include <StringBuf.h>
#include <wx/Colour.h>
#include <wx/Brush.h>
#include <wx/BoxSizer.h>
#include <wx/Sizer.h>
#include <haxe/Log.h>
#include <Reflect.h>
#include <wx/ComboBox.h>
#include <wx/ControlWithItems.h>
#include <wx/Window.h>
#include <wx/EventHandler.h>
#include <wx/App.h>
#include <cpp/Lib.h>
#include <wx/Loader.h>

void __boot_all()
{
hx::RegisterResources( hx::GetResources() );
::wx::TextCtrl_obj::__register();
::wx::StaticText_obj::__register();
::IntHash_obj::__register();
::haxe::Resource_obj::__register();
::Type_obj::__register();
::ValueType_obj::__register();
::wx::Bitmap_obj::__register();
::ApplicationMain_obj::__register();
::Simple_obj::__register();
::Std_obj::__register();
::wx::DC_obj::__register();
::wx::Pen_obj::__register();
::haxe::io::Error_obj::__register();
::wx::MenuBar_obj::__register();
::wx::Button_obj::__register();
::IntIter_obj::__register();
::haxe::io::Bytes_obj::__register();
::wx::Panel_obj::__register();
::wx::EventID_obj::__register();
::wx::Menu_obj::__register();
::wx::Error_obj::__register();
::wx::Frame_obj::__register();
::wx::TopLevelWindow_obj::__register();
::wx::FlexGridSizer_obj::__register();
::wx::GridSizer_obj::__register();
::wx::Font_obj::__register();
::StringBuf_obj::__register();
::wx::Colour_obj::__register();
::wx::Brush_obj::__register();
::wx::BoxSizer_obj::__register();
::wx::Sizer_obj::__register();
::haxe::Log_obj::__register();
::Reflect_obj::__register();
::wx::ComboBox_obj::__register();
::wx::ControlWithItems_obj::__register();
::wx::Window_obj::__register();
::wx::EventHandler_obj::__register();
::wx::App_obj::__register();
::cpp::Lib_obj::__register();
::wx::Loader_obj::__register();
::cpp::Lib_obj::__boot();
::haxe::Log_obj::__boot();
::wx::Loader_obj::__boot();
::wx::App_obj::__boot();
::wx::EventHandler_obj::__boot();
::wx::Window_obj::__boot();
::wx::ControlWithItems_obj::__boot();
::wx::ComboBox_obj::__boot();
::Reflect_obj::__boot();
::wx::Sizer_obj::__boot();
::wx::BoxSizer_obj::__boot();
::wx::Brush_obj::__boot();
::wx::Colour_obj::__boot();
::StringBuf_obj::__boot();
::wx::Font_obj::__boot();
::wx::GridSizer_obj::__boot();
::wx::FlexGridSizer_obj::__boot();
::wx::TopLevelWindow_obj::__boot();
::wx::Frame_obj::__boot();
::wx::Error_obj::__boot();
::wx::Menu_obj::__boot();
::wx::EventID_obj::__boot();
::wx::Panel_obj::__boot();
::haxe::io::Bytes_obj::__boot();
::IntIter_obj::__boot();
::wx::Button_obj::__boot();
::wx::MenuBar_obj::__boot();
::haxe::io::Error_obj::__boot();
::wx::Pen_obj::__boot();
::wx::DC_obj::__boot();
::Std_obj::__boot();
::Simple_obj::__boot();
::ApplicationMain_obj::__boot();
::wx::Bitmap_obj::__boot();
::ValueType_obj::__boot();
::Type_obj::__boot();
::haxe::Resource_obj::__boot();
::IntHash_obj::__boot();
::wx::StaticText_obj::__boot();
::wx::TextCtrl_obj::__boot();
}

