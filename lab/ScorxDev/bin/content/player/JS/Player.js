...
mloader.ImageLoader.prototype = $extend(mloader.LoaderBase.prototype,{
	...
	,loaderLoad: function() {
		this.content = js.Browser.document.createElement("img");
...
nme.AssetData.initialize = function() {
	if(!nme.AssetData.initialized) nme.AssetData.initialized = true;
}
var openfl = {}
...
player.AppMediator.prototype = $extend(sx.mvc.app.base.AppBaseMediator.prototype,{
	loadContent: function() {
		...
		loadParameters.host = this.config.host;
		loadParameters.productId = this.config.productId;
		loadParameters.userId = this.config.userId;
		loadParameters.type = scorx.types.ScoreLoadingType.screen;
		Debug.log(loadParameters);
		this.loadPages.dispatch(loadParameters);
		this.loadPages.status.add(function(status) {
			var $e = (status);
			switch( $e[1] ) {
			case 0:
				var nrOfPages = $e[2];
...
scorx.controller.LoadPagesCommand.prototype = $extend(mmvc.impl.Command.prototype,{
	execute: function() {
		...
		this.loader.onPageLoaded = function(pageNr,nrOfPages,data,type) {
			console.log("loader.onPageLoaded " + pageNr + "/" + nrOfPages);
			if(pageNr == 0) {
