...
class ApplicationMain
{
	...
	{
		#if openfl
		flash.Lib.setPackage("", "Player", "player.Player", "1.0.0");
		...
		wx.App.boot(function()
		{
			...
			frame = wx.Frame.create(null, null, "Player", null, { width: 800, height: 480 });
			...
			var hasMain = false;
			for (methodName in Type.getClassFields(player.Main)) {
				if (methodName == "main") {
					hasMain = true;
					break;
				}
			...
			if (hasMain) {
				Reflect.callMethod(player.Main, Reflect.field(player.Main, "main"), []);
			}else {
				var instance = Type.createInstance(player.Main, []);
			}
			
			/*if (Reflect.hasField(player.Main, "main")) {
				player.Main.main();			
			}*/
...
class DocumentClass extends player.Main {}
