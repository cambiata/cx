...
ApplicationMain.main = function() {
	...
	ApplicationMain.loaders.set("img/checkboxChecked.png",loader);
...
ApplicationMain.preloader_onComplete = function(event) {
	...
	if(Reflect.field(player.Main,"main") == null) {
	...
	} else Reflect.field(player.Main,"main").apply(player.Main,[]);
}
var flash = {}
flash.events = {}
...
$hxClasses["sx.mvc.MvcMain"] = sx.mvc.MvcMain;
sx.mvc.MvcMain.__name__ = ["sx","mvc","MvcMain"];
sx.mvc.MvcMain.__super__ = flash.display.Sprite;
sx.mvc.MvcMain.prototype = $extend(flash.display.Sprite.prototype,{
	added: function(e) {
	...
	,resize: function(e) {
		if(!this.inited) this.initApp();
	}
	,inited: null
	,__class__: sx.mvc.MvcMain
});
var player = {}
player.Main = function() {
	sx.mvc.MvcMain.call(this);
};
$hxClasses["player.Main"] = player.Main;
player.Main.__name__ = ["player","Main"];
player.Main.main = function() {
	flash.Lib.get_current().addChild(new player.Main());
}
player.Main.__super__ = sx.mvc.MvcMain;
player.Main.prototype = $extend(sx.mvc.MvcMain.prototype,{
	init: function() {
		this.addChild(new player.AppContext().view);
	}
	,__class__: player.Main
});
var DocumentClass = function() {
	player.Main.call(this);
};
$hxClasses["DocumentClass"] = DocumentClass;
DocumentClass.__name__ = ["DocumentClass"];
DocumentClass.__super__ = player.Main;
DocumentClass.prototype = $extend(player.Main.prototype,{
	get_stage: function() {
...
cx.flash.layout.DocumentLayout.arrange = function(holder,holderWidth,childWidth,childHeight,tweenTime) {
	...
	var result = { holderRect : new flash.geom.Rectangle(0,0,holder.get_w(),holder.get_h()), pageSize : new flash.geom.Point(childWidth,childHeight), pageInfos : new Array()};
	var _g = 0;
	while(_g < numChildren) {
		var i = _g++;
		var pageInfo = { rect : new flash.geom.Rectangle(childX[i],childY[i],childWidth,childHeight)};
...
cx.flash.ui = {}
cx.flash.ui.UI = function() { }
$hxClasses["cx.flash.ui.UI"] = cx.flash.ui.UI;
cx.flash.ui.UI.__name__ = ["cx","flash","ui","UI"];
cx.flash.ui.UI.createButton = function(txt,onClick,x,y,color,width,height,margin,format,round) {
	if(round == null) round = false;
	if(margin == null) margin = 4;
	if(height == null) height = 0;
	if(width == null) width = 0;
	if(color == null) color = 16763904;
	if(y == null) y = 0;
	if(x == null) x = 0;
	var textField = new flash.text.TextField();
	if(format != null) textField.set_defaultTextFormat(format);
	textField.set_text(txt);
	textField.set_autoSize("LEFT");
	textField.selectable = false;
	if(width == 0) width = textField.get_width() + margin;
	if(height == 0) height = textField.get_height() + margin;
	textField.set_x(width / 2 - textField.get_width() / 2);
	textField.set_y(height / 2 - textField.get_height() / 2);
	var btn = new flash.display.Sprite();
	btn.get_graphics().beginFill(color);
	if(!round) btn.get_graphics().drawRoundRect(0,0,width,height,9); else btn.get_graphics().drawEllipse(0,0,width,height);
	if(onClick != null) {
		btn.buttonMode = true;
		btn.mouseChildren = false;
		btn.set_useHandCursor(true);
		btn.addEventListener(flash.events.MouseEvent.CLICK,function(e) {
			onClick();
		});
	}
...
cx.flash.ui.UI.createText = function(txt,x,y,format) {
	if(y == null) y = 0;
	if(x == null) x = 0;
	var textField = new flash.text.TextField();
	if(format != null) textField.set_defaultTextFormat(format);
...
cx.layout.Vertical.STRETCH_MARGIN = function(topMargin,bottomMargin) { var $x = ["STRETCH_MARGIN",7,topMargin,bottomMargin]; $x.__enum__ = cx.layout.Vertical; $x.toString = $estr; return $x; }
...
mloader.ImageLoader = function(url) {
	mloader.LoaderBase.call(this,url);
	this.contentLoaderInfo = flash.display.LoaderInfo.create(null);
	this.bitmapData = new flash.display.BitmapData(0,0);
	this.contentLoaderInfo.addEventListener(flash.events.ProgressEvent.PROGRESS,$bind(this,this.loaderProgressed));
	this.contentLoaderInfo.addEventListener(flash.events.Event.COMPLETE,$bind(this,this.loaderCompleted));
	this.contentLoaderInfo.addEventListener(flash.events.IOErrorEvent.IO_ERROR,$bind(this,this.loaderErrored));
};
...
mloader.ImageLoader.prototype = $extend(mloader.LoaderBase.prototype,{
	...
	,loaderLoad: function() {
		this.bitmapData.nmeLoadFromFile(this.url,this.contentLoaderInfo);
	}
...
nme.AssetData.initialize = function() {
	if(!nme.AssetData.initialized) {
		nme.AssetData.path.set("img/checkboxChecked.png","img/checkboxChecked.png");
		var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
		nme.AssetData.type.set("img/checkboxChecked.png",value);
...
$hxClasses["sx.mvc.app.base.AppBaseMediator"] = sx.mvc.app.base.AppBaseMediator;
sx.mvc.app.base.AppBaseMediator.__name__ = ["sx","mvc","app","base","AppBaseMediator"];
sx.mvc.app.base.AppBaseMediator.__super__ = mmvc.impl.Mediator;
sx.mvc.app.base.AppBaseMediator.prototype = $extend(mmvc.impl.Mediator.prototype,{
	register: function() {
	}
	,onRegister: function() {
		this.register();
	}
	,__class__: sx.mvc.app.base.AppBaseMediator
});
player.AppMediator = function() {
	sx.mvc.app.base.AppBaseMediator.call(this);
};
$hxClasses["player.AppMediator"] = player.AppMediator;
player.AppMediator.__name__ = ["player","AppMediator"];
player.AppMediator.__super__ = sx.mvc.app.base.AppBaseMediator;
player.AppMediator.prototype = $extend(sx.mvc.app.base.AppBaseMediator.prototype,{
	...
	,register: function() {
		...
		this.viewScrollerView = new player.view.ScrollerView();
		this.view.addChild(this.viewScrollerView);
		this.viewPlaybackMixerView = new player.view.PlaybackMixerView();
		this.view.addChild(this.viewPlaybackMixerView);
		this.viewErrordisplayView = new player.view.ErrordisplayView();
		this.view.addChild(this.viewErrordisplayView);
		...
		this.layoutManager.add(new cx.layout.WidgetItem(this.viewScrollerView,cx.layout.Horizontal.STRETCH_MARGIN(40,60),cx.layout.Vertical.BOTTOM));
		this.layoutManager.add(new cx.layout.WidgetItem(this.zoomView,cx.layout.Horizontal.RIGHT,cx.layout.Vertical.TOP));
		this.layoutManager.add(new cx.layout.WidgetItem(this.viewPlaybackMixerView,cx.layout.Horizontal.RIGHT,cx.layout.Vertical.TOP_MARGIN(100)));
		this.layoutManager.add(new cx.layout.WidgetItem(this.viewErrordisplayView,cx.layout.Horizontal.CENTER,cx.layout.Vertical.CENTER));
		var pagesItem = new cx.layout.WidgetItem(this.pagesView,cx.layout.Horizontal.STRETCH_MARGIN(0,100),cx.layout.Vertical.STRETCH_MARGIN(0,30));
		...
		this.mediate(this.config.updated.add(function() {
			_g.pagesLoader.load(_g.config.host,_g.config.productId,_g.config.userId);
			if(_g.config.playbackLevel == scorx.model.AccessLevelPlay.FullPlayback) {
				_g.gridLoader.load(_g.config.host,_g.config.productId,_g.config.userId);
				_g.channelsLoader.load(_g.config.host,_g.config.productId,_g.config.userId);
			}
		}));
		this.channelsLoader.result.add(function(result) {
			var $e = (result);
			switch( $e[1] ) {
			case 3:
				var url = $e[3], message = $e[2];
				_g.errors.addError(message + " Url: " + url);
				break;
			default:
			}
		});
		this.pagesLoader.result.add(function(result) {
			var $e = (result);
			switch( $e[1] ) {
			case 3:
				var url = $e[3], message = $e[2];
		...
		this.gridLoader.result.add(function(result) {
			var $e = (result);
			switch( $e[1] ) {
			case 1:
				var url = $e[3], message = $e[2];
	...
	,viewErrordisplayView: null
	,viewPlaybackMixerView: null
	,viewScrollerView: null
	,zoomView: null
	,pagesView: null
...
sx.mvc.app.base.AppBaseContext = function() {
	...
	mmvc.impl.Context.call(this,view);
	this.view = view;
};
$hxClasses["sx.mvc.app.base.AppBaseContext"] = sx.mvc.app.base.AppBaseContext;
sx.mvc.app.base.AppBaseContext.__name__ = ["sx","mvc","app","base","AppBaseContext"];
sx.mvc.app.base.AppBaseContext.__super__ = mmvc.impl.Context;
sx.mvc.app.base.AppBaseContext.prototype = $extend(mmvc.impl.Context.prototype,{
	init: function() {
	}
	,config: function() {
	}
	...
	,view: null
	,__class__: sx.mvc.app.base.AppBaseContext
});
player.AppContext = function() {
	sx.mvc.app.base.AppBaseContext.call(this);
};
$hxClasses["player.AppContext"] = player.AppContext;
player.AppContext.__name__ = ["player","AppContext"];
...
player.AppContext.prototype = $extend(sx.mvc.app.base.AppBaseContext.prototype,{
	init: function() {
		...
		this.get_injector().mapSingleton(scorx.data.GridProc);
		this.get_injector().mapSingleton(sx.player.grid.PageSystemsUtils);
		this.get_injector().mapSingleton(scorx.model.PlayPosition);
		this.get_injector().mapSingleton(scorx.data.GridLoader);
		this.get_injector().mapSingleton(scorx.data.PagesLoader);
		this.get_injector().mapSingleton(scorx.data.ChannelsLoader);
		this.get_injector().mapSingleton(scorx.model.PlaybackEngine);
		this.get_injector().mapSingleton(scorx.data.Errors);
		this.get_injector().mapSingleton(scorx.data.Zooma);
		...
		this.get_mediatorMap().mapView(player.view.ScrollerView,player.view.ScrollerMediator);
		this.get_mediatorMap().mapView(player.view.PlaybackMixerView,player.view.PlaybackMixerMediator);
		this.get_mediatorMap().mapView(player.view.ErrordisplayView,player.view.ErrordisplayMediator);
	...
	,config: function() {
		(function() {
			...
			ru.stablex.ui.UIBuilder.defaults.get("Button").set("Settings",function(__ui__widget0) {
				var __ui__widget1 = js.Boot.__cast(__ui__widget0 , ru.stablex.ui.widgets.Button);
				__ui__widget1.childPadding = 0;
				__ui__widget1.apart = true;
				__ui__widget1.autoWidth = true;
				__ui__widget1.get_ico().set_src("ui/android/img/ico/dark/settings.png");
				__ui__widget1.set_icoBeforeLabel(false);
			});
			...
			ru.stablex.ui.UIBuilder.defaults.get("Scroll").set("Default",function(__ui__widget0) {
				...
				(js.Boot.__cast(__ui__widget11.skin , ru.stablex.ui.skins.Paint)).color = 14540253;
...
ru.stablex.ui.widgets.Widget.prototype = $extend(ru.stablex.TweenSprite.prototype,{
	...
	,set_id: function(id) {
		if(id == null) {
			console.log("Widget id cannot be null");
			throw "Widget id cannot be null";
		}
		if(this.id != null) ru.stablex.ui.UIBuilder._objects.remove(this.id);
		this.id = id;
		if(ru.stablex.ui.UIBuilder._objects.exists(this.id)) ru.stablex.Err.trigger("Widget id \"" + this.id + "\" is already used"); else ru.stablex.ui.UIBuilder._objects.set(this.id,this);
		return id;
	}
	...
	,set_skinName: function(sn) {
		this.skin = ((function($this) {
			var $r;
			if(!ru.stablex.ui.UIBuilder.skins.exists(sn)) ru.stablex.Err.trigger("Skin not found: " + sn);
			$r = ru.stablex.ui.UIBuilder.skins.get(sn);
			return $r;
		}(this)))();
		if(this.created) {
...
player.view.ConfigurationViewView.prototype = $extend(sx.mvc.view.VBoxView.prototype,{
	addSkin: function() {
		...
		skin.alpha = 1;
		skin.border = 1;
		skin.borderColor = 14540253;
		skin.apply(this);
	}
	,createChildren: function() {
		this.childPadding = 0;
		this.align = "top,left";
		var format = new flash.text.TextFormat("Arial",10);
		this.labelUserId = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Text);
		this.labelUserId.set_h(14);
		...
		this.labelPlaybackLevel.set_h(14);
		this.labelPlaybackLevel.set_text("PlaybackLevel:");
		this.labelPlaybackLevel.format = format;
		this.addChild(this.labelPlaybackLevel);
		this.labelViewLevel = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Text);
		this.labelViewLevel.set_h(14);
		this.labelViewLevel.set_text("ViewLevel:");
...
player.view.ConfigurationViewMediator.prototype = $extend(mmvc.impl.Mediator.prototype,{
	onRegister: function() {
		...
		this.mediate(this.config.updated.add(function() {
			...
			_g.view.labelViewLevel.set_text("ViewLevel: " + Std.string(_g.config.viewLevel));
		}));
	}
	,config: null
	,__class__: player.view.ConfigurationViewMediator
});
sx.mvc.view.WidgetView = function() {
	var _g = this;
	ru.stablex.ui.widgets.Widget.call(this);
	this.createChildren();
	this.addSkin();
...
$hxClasses["sx.mvc.view.WidgetView"] = sx.mvc.view.WidgetView;
sx.mvc.view.WidgetView.__name__ = ["sx","mvc","view","WidgetView"];
sx.mvc.view.WidgetView.__super__ = ru.stablex.ui.widgets.Widget;
sx.mvc.view.WidgetView.prototype = $extend(ru.stablex.ui.widgets.Widget.prototype,{
	addSkin: function() {
		console.log("W as - should be overridden");
	}
	,createChildren: function() {
		console.log("W cc - should be overridden");
		var label = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Text);
		label.set_text("This is WidgetView");
		label.set_w(200);
		this.addChild(label);
	}
	,__class__: sx.mvc.view.WidgetView
});
player.view.ErrordisplayView = function() {
	sx.mvc.view.WidgetView.call(this);
};
$hxClasses["player.view.ErrordisplayView"] = player.view.ErrordisplayView;
player.view.ErrordisplayView.__name__ = ["player","view","ErrordisplayView"];
player.view.ErrordisplayView.__super__ = sx.mvc.view.WidgetView;
player.view.ErrordisplayView.prototype = $extend(sx.mvc.view.WidgetView.prototype,{
	addSkin: function() {
		var skin = new ru.stablex.ui.skins.Paint();
		skin.color = 8388608;
		skin.corners = [8,8];
	...
	,createChildren: function() {
		this.set_w(500);
		this.set_h(260);
		this.btn = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Button);
		this.btn.set_text("Close");
		this.addChild(this.btn);
		this.btn.set_y(this.get_h() - 40);
		this.btn.set_x(this.get_w() - 110);
		this.tf = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Text);
		this.tf.format = new flash.text.TextFormat("Arial",12,16777215);
		this.tf.set_text("Error");
		this.tf.set_x(this.tf.set_y(10));
		this.addChild(this.tf);
		this.set_visible(false);
	}
	,tf: null
	,btn: null
	,__class__: player.view.ErrordisplayView
});
player.view.ErrordisplayMediator = function() {
	mmvc.impl.Mediator.call(this);
};
$hxClasses["player.view.ErrordisplayMediator"] = player.view.ErrordisplayMediator;
player.view.ErrordisplayMediator.__name__ = ["player","view","ErrordisplayMediator"];
player.view.ErrordisplayMediator.__super__ = mmvc.impl.Mediator;
player.view.ErrordisplayMediator.prototype = $extend(mmvc.impl.Mediator.prototype,{
	onRegister: function() {
		var _g = this;
		console.log("ErrordisplayMediator registered");
		this.view.btn.onPress = function(e) {
			_g.view.set_visible(false);
		};
		this.errors.status.add(function(status) {
			var $e = (status);
			switch( $e[1] ) {
			case 1:
				var messages = $e[3], message = $e[2];
				_g.view.set_visible(true);
				_g.view.tf.set_text("");
				var _g1 = 0;
				while(_g1 < messages.length) {
					var mess = messages[_g1];
					++_g1;
					var _g2 = _g.view.tf;
					_g2.set_text(_g2.get_text() + (mess + "\n"));
				}
				_g.view.tf.refresh();
				break;
			case 0:
	...
	,errors: null
	,__class__: player.view.ErrordisplayMediator
});
player.view.PlaybackMixerView = function() {
	sx.mvc.view.VBoxView.call(this);
};
$hxClasses["player.view.PlaybackMixerView"] = player.view.PlaybackMixerView;
player.view.PlaybackMixerView.__name__ = ["player","view","PlaybackMixerView"];
player.view.PlaybackMixerView.__super__ = sx.mvc.view.VBoxView;
player.view.PlaybackMixerView.prototype = $extend(sx.mvc.view.VBoxView.prototype,{
	setSkin: function() {
		var skin = new ru.stablex.ui.skins.Paint();
		skin.color = 14540253;
		skin.alpha = 1;
	...
	,createChildren: function() {
		...
		this.set_w(100);
		this.set_h(320);
		this.playSkin = new ru.stablex.ui.skins.Paint();
		this.playSkin.color = sx.ScorxColors.ScorxGreenDark;
		this.playSkin.corners = [8];
		this.playSkinHover = new ru.stablex.ui.skins.Paint();
		this.playSkinHover.color = sx.ScorxColors.ScorxGreen;
		this.playSkinHover.corners = [8];
		this.stopSkin = new ru.stablex.ui.skins.Paint();
		this.stopSkin.color = sx.ScorxColors.ScorxRed;
		this.stopSkin.corners = [8];
		this.stopSkinHover = new ru.stablex.ui.skins.Paint();
		this.stopSkinHover.color = sx.ScorxColors.ScorxRedLight;
		this.stopSkinHover.corners = [8];
		this.btnWidget = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Widget);
		this.btnWidget.set_h(30);
		this.btnWidget.set_w(this.get_w());
		this.addChild(this.btnWidget);
		this.btn = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Button);
		this.btn.set_x(10);
		this.btn.format = new flash.text.TextFormat("Arial",12,16777215);
		this.btn.set_text("Play");
		this.btn.set_w(80);
	...
	,playSkin: null
	,progress: null
	,__class__: player.view.PlaybackMixerView
});
player.view.PlaybackMixerMediator = function() {
	mmvc.impl.Mediator.call(this);
};
$hxClasses["player.view.PlaybackMixerMediator"] = player.view.PlaybackMixerMediator;
player.view.PlaybackMixerMediator.__name__ = ["player","view","PlaybackMixerMediator"];
player.view.PlaybackMixerMediator.__super__ = mmvc.impl.Mediator;
player.view.PlaybackMixerMediator.prototype = $extend(mmvc.impl.Mediator.prototype,{
	onRegister: function() {
		var _g = this;
		console.log("PlaybackMixerMediator registered");
		this.view.btn.onPress = function(e) {
			if(_g.playbackEngine.isPlaying()) _g.playbackEngine.stop(); else _g.playbackEngine.start(_g.playPosition.getPosition());
		};
		this.channelsLoader.result.add(function(result) {
			var $e = (result);
			switch( $e[1] ) {
			case 0:
				var channels = $e[2];
				var _g1 = 0;
				while(_g1 < channels.length) {
					var channel = channels[_g1];
					++_g1;
					var channelWidget = new scorx.ui.ChannelWidget(channel.Label,channel.Id);
					_g.view.addChild(channelWidget);
					_g.view.refresh();
					channelWidget.onChangeValue = ($_=_g.playbackEngine,$bind($_,$_.setVolume));
				}
				break;
			case 2:
				var channelsData = $e[2];
				_g.view.btn.set_visible(true);
				break;
			default:
			}
		});
		this.playbackEngine.onStart.add(function(pos) {
			_g.view.btn.set_text("Stop");
			_g.view.btn.skin = _g.view.stopSkin;
			_g.view.btn.skinHovered = _g.view.stopSkinHover;
			_g.view.btn.refresh();
		});
		this.playbackEngine.onStop.add(function() {
			_g.view.btn.set_text("Start");
			_g.view.btn.skin = _g.view.playSkin;
			_g.view.btn.skinHovered = _g.view.playSkinHover;
			_g.view.btn.refresh();
		});
	}
	,playPosition: null
	,channelsLoader: null
	,playbackEngine: null
	,__class__: player.view.PlaybackMixerMediator
});
...
player.view.ScrollerView = function() {
	sx.mvc.view.HBoxView.call(this);
};
$hxClasses["player.view.ScrollerView"] = player.view.ScrollerView;
player.view.ScrollerView.__name__ = ["player","view","ScrollerView"];
player.view.ScrollerView.__super__ = sx.mvc.view.HBoxView;
player.view.ScrollerView.prototype = $extend(sx.mvc.view.HBoxView.prototype,{
	addSkin: function() {
	}
	,createChildren: function() {
		this.progress = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Progress);
		this.progress.set_interactive(true);
		this.progress.set_w(400);
		this.progress.set_h(16);
		this.addChild(this.progress);
		this.progress.set_useHandCursor(true);
		this.txtPos = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Text);
		this.txtPos.set_text("0:00");
		this.addChild(this.txtPos);
		this.btnGrid = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Button);
		this.btnGrid.set_text("grid");
		this.btnGrid.set_w(140);
		this.addChild(this.btnGrid);
		this.btnGrid.set_visible(false);
		this.btnScreen = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Button);
		this.btnScreen.set_text("screen");
		this.btnScreen.set_w(140);
		this.addChild(this.btnScreen);
		this.btnScreen.set_visible(false);
		this.btnSound = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Button);
		this.btnSound.set_text("sound");
		this.btnSound.set_w(140);
		this.addChild(this.btnSound);
		this.btnSound.set_visible(false);
		this.btnPlay = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Button);
		this.btnPlay.set_text("Play");
		this.btnPlay.set_w(80);
		this.addChild(this.btnPlay);
		this.btnPlay.set_visible(false);
		this.btnStop = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Button);
		this.btnStop.set_text("Stop");
		this.btnStop.set_w(80);
	...
	,btnGrid: null
	,progress: null
	,__class__: player.view.ScrollerView
});
player.view.ScrollerMediator = function() {
	mmvc.impl.Mediator.call(this);
};
$hxClasses["player.view.ScrollerMediator"] = player.view.ScrollerMediator;
player.view.ScrollerMediator.__name__ = ["player","view","ScrollerMediator"];
player.view.ScrollerMediator.__super__ = mmvc.impl.Mediator;
player.view.ScrollerMediator.prototype = $extend(mmvc.impl.Mediator.prototype,{
	onRegister: function() {
		var _g = this;
		console.log("ScrollerMediator registered");
		this.view.progress.addEventListener("widgetChange",function(e) {
		});
		this.view.progress.addEventListener(flash.events.MouseEvent.MOUSE_MOVE,function(e) {
			var value = _g.view.progress.get_value() / 100;
			_g.playPosition.setPosition(value,_g.view.progress);
		});
		this.view.progress.addEventListener(flash.events.MouseEvent.MOUSE_UP,function(e) {
			_g.playbackEngine.stop();
			var value = _g.view.progress.get_value() / 100;
			_g.playPosition.setPosition(value,_g.view.progress);
		});
		this.mediate(this.playPosition.positionSignal.add(function(info) {
			if(Std.string(info.sender) != Std.string(_g.view.progress)) _g.view.progress.set_value(info.pos * 100);
			var seconds = _g.playbackEngine.getSoundLength();
			...
			_g.view.txtPos.set_text("" + min + ":" + sec);
		}));
		this.mediate(this.gridLoader.result.add(function(result) {
			var $e = (result);
			switch( $e[1] ) {
			case 0:
				var xmlString = $e[2];
		...
		this.mediate(this.pagesLoader.result.add(function(result) {
			var $e = (result);
			switch( $e[1] ) {
			case 0:
				var nrOfPages = $e[2];
				console.log("STARTED " + nrOfPages);
				break;
			default:
			}
		}));
		this.channelsLoader.result.add(function(result) {
			...
			case 0:
				...
				break;
			case 1:
				var tag = $e[3], data = $e[2];
				break;
			case 2:
				var channelsData = $e[2];
				console.log("ChannelsData complete JS");
				var url = "" + _g.configuration.host + "media/channel/" + _g.configuration.productId + "/200/" + _g.configuration.userId;
				console.log(url);
				_g.playbackEngine.loadOneFile(url);
				break;
			default:
			}
		});
		this.view.btnGrid.onPress = function(e) {
			console.log("press");
			_g.gridLoader.load(_g.configuration.host,_g.configuration.productId,_g.configuration.userId);
		};
		this.view.btnScreen.onPress = function(e) {
			console.log("press");
			_g.pagesLoader.load(_g.configuration.host,_g.configuration.productId,_g.configuration.userId);
		};
		this.view.btnSound.onPress = function(e) {
			_g.channelsLoader.load(_g.configuration.host,_g.configuration.productId,_g.configuration.userId);
		};
		this.view.btnPlay.onPress = function(e) {
			_g.playbackEngine.start(_g.playPosition.getPosition());
			_g.errors.addError("This is an error!");
		};
		this.view.btnStop.onPress = function(e) {
			_g.playbackEngine.stop();
			_g.errors.clear();
		};
		this.playbackEngine.onPosition.add(function(position) {
			console.log("position " + position);
			_g.playPosition.setPosition(position,_g);
		});
		this.playbackEngine.onStart.add(function(position) {
			console.log("onStart " + position);
		});
	}
	,errors: null
	,playbackEngine: null
	...
	,configuration: null
	,playPosition: null
	,__class__: player.view.ScrollerMediator
});
player.view.TestBitmap = function(width,height,transparent,fillRGBA,onload) {
	...
	if(player.view.TestBitmap.preload != null) {
		this._nmeTextureBuffer.width = player.view.TestBitmap.preload.get_width() | 0;
		this._nmeTextureBuffer.height = player.view.TestBitmap.preload.get_height() | 0;
		this.rect = new flash.geom.Rectangle(0,0,player.view.TestBitmap.preload.get_width(),player.view.TestBitmap.preload.get_height());
		this.setPixels(this.rect,player.view.TestBitmap.preload.getPixels(this.rect));
		this.nmeLease.set(this.nmeLeaseNum++,new Date().getTime());
	} else {
		var byteArray = flash.utils.ByteArray.fromBytes(haxe.Resource.getBytes(player.view.TestBitmap.resourceName));
		if(onload != null && !js.Boot.__instanceof(onload,Bool)) this.nmeLoadFromBytes(byteArray,null,onload); else this.nmeLoadFromBytes(byteArray,null,null);
	}
};
$hxClasses["player.view.TestBitmap"] = player.view.TestBitmap;
player.view.TestBitmap.__name__ = ["player","view","TestBitmap"];
player.view.TestBitmap.__super__ = flash.display.BitmapData;
player.view.TestBitmap.prototype = $extend(flash.display.BitmapData.prototype,{
	__class__: player.view.TestBitmap
});
...
player.view.ZoomView.prototype = $extend(sx.mvc.view.VBoxView.prototype,{
	createButton: function(text) {
		...
		btn.set_w(80);
		btn.set_text(text);
		btn.format = new flash.text.TextFormat("Arial",12,16777215);
		var skin1 = new ru.stablex.ui.skins.Paint();
		skin1.color = sx.ScorxColors.ScorxPetrol;
		skin1.corners = [0];
		var skin2 = new ru.stablex.ui.skins.Img();
		skin2.set_bitmapData(new player.view.TestBitmap(0,0));
...
player.view.ZoomMediator.prototype = $extend(mmvc.impl.Mediator.prototype,{
	onRegister: function() {
		...
		this.view.btnFull.onPress = function(e) {
			if(_g.zooma.getZoom() == sx.mvc.view.enums.ScrollWidgetZoom.ZoomFullHeight) _g.zooma.setZoom(sx.mvc.view.enums.ScrollWidgetZoom.Zoom40); else _g.zooma.setZoom(sx.mvc.view.enums.ScrollWidgetZoom.ZoomFullHeight);
		};
		this.zooma.zoomSignal.add(function(zoom) {
			switch( (zoom)[1] ) {
			case 1:
				_g.view.btnFull.set_text("To small");
				break;
			default:
				_g.view.btnFull.set_text("To full");
			}
		});
...
ru.stablex.ui.UIBuilder.skin = function(skinName) {
	if(!ru.stablex.ui.UIBuilder.skins.exists(skinName)) ru.stablex.Err.trigger("Skin not found: " + skinName);
...
ru.stablex.ui.UIBuilder.save = function(obj) {
	if(ru.stablex.ui.UIBuilder._objects.exists(obj.id)) ru.stablex.Err.trigger("Widget id \"" + obj.id + "\" is already used"); else ru.stablex.ui.UIBuilder._objects.set(obj.id,obj);
}
...
ru.stablex.ui.misc.BtnState.prototype = {
	set_skinName: function(s) {
		this.skin = ((function($this) {
			var $r;
			if(!ru.stablex.ui.UIBuilder.skins.exists(s)) ru.stablex.Err.trigger("Skin not found: " + s);
			$r = ru.stablex.ui.UIBuilder.skins.get(s);
			return $r;
		}(this)))();
...
ru.stablex.ui.skins.Img = function() {
	this._bitmapData = null;
	this._src = null;
	ru.stablex.ui.skins.Skin.call(this);
};
$hxClasses["ru.stablex.ui.skins.Img"] = ru.stablex.ui.skins.Img;
ru.stablex.ui.skins.Img.__name__ = ["ru","stablex","ui","skins","Img"];
ru.stablex.ui.skins.Img.__super__ = ru.stablex.ui.skins.Skin;
ru.stablex.ui.skins.Img.prototype = $extend(ru.stablex.ui.skins.Skin.prototype,{
	...
	,draw: function(w) {
		...
		if(bmp == null && this._src != null) {
			...
			if(bmp == null) ru.stablex.Err.trigger("Bitmap not found: " + this._src);
		} else if(bmp == null) {
			console.log("Bitmap is not specified");
			throw "Bitmap is not specified";
		}
		if(w.get_w() != (bmp._nmeTextureBuffer != null?bmp._nmeTextureBuffer.width:0) || w.get_h() != (bmp._nmeTextureBuffer != null?bmp._nmeTextureBuffer.height:0)) w.resize(bmp._nmeTextureBuffer != null?bmp._nmeTextureBuffer.width:0,bmp._nmeTextureBuffer != null?bmp._nmeTextureBuffer.height:0);
		w.get_graphics().beginBitmapFill(bmp,null,false,this.smooth);
		w.get_graphics().drawRect(0,0,bmp._nmeTextureBuffer != null?bmp._nmeTextureBuffer.width:0,bmp._nmeTextureBuffer != null?bmp._nmeTextureBuffer.height:0);
		w.get_graphics().endFill();
	}
	,smooth: null
	,_bitmapData: null
	,_src: null
	,__class__: ru.stablex.ui.skins.Img
	,__properties__: {set_src:"set_src",get_src:"get_src",set_bitmapData:"set_bitmapData",get_bitmapData:"get_bitmapData"}
});
ru.stablex.ui.skins.Layer = function() {
	ru.stablex.ui.skins.Skin.call(this);
};
$hxClasses["ru.stablex.ui.skins.Layer"] = ru.stablex.ui.skins.Layer;
ru.stablex.ui.skins.Layer.__name__ = ["ru","stablex","ui","skins","Layer"];
ru.stablex.ui.skins.Layer.__super__ = ru.stablex.ui.skins.Skin;
ru.stablex.ui.skins.Layer.prototype = $extend(ru.stablex.ui.skins.Skin.prototype,{
	apply: function(w) {
		if(this.clear) w.get_graphics().clear();
		if(this.behind != null) {
	...
	,behind: null
	,current: null
	,__class__: ru.stablex.ui.skins.Layer
});
...
ru.stablex.ui.widgets.Button.prototype = $extend(ru.stablex.ui.widgets.Text.prototype,{
	...
	,set_skinPressedName: function(s) {
		this.skinPressed = ((function($this) {
			var $r;
			if(!ru.stablex.ui.UIBuilder.skins.exists(s)) ru.stablex.Err.trigger("Skin not found: " + s);
			$r = ru.stablex.ui.UIBuilder.skins.get(s);
			return $r;
	...
	,set_skinHoveredName: function(s) {
		this.skinHovered = ((function($this) {
			var $r;
			if(!ru.stablex.ui.UIBuilder.skins.exists(s)) ru.stablex.Err.trigger("Skin not found: " + s);
			$r = ru.stablex.ui.UIBuilder.skins.get(s);
			return $r;
...
ru.stablex.ui.widgets.Options.prototype = $extend(ru.stablex.ui.widgets.Button.prototype,{
	...
	,set_options: function(o) {
		if(o == null || o.length == 0) {
			console.log("Option list must not be null or empty");
			throw "Option list must not be null or empty";
		}
...
scorx.audio = {}
scorx.audio.js = {}
scorx.audio.js.Mixer = function(playCallback) {
	this.posStop = 0;
	this.posStart = 0;
	this.posCurrent = 0;
};
$hxClasses["scorx.audio.js.Mixer"] = scorx.audio.js.Mixer;
scorx.audio.js.Mixer.__name__ = ["scorx","audio","js","Mixer"];
scorx.audio.js.Mixer.prototype = {
	onError: function(msg) {
	}
	,onStop: function() {
	}
	,onStart: function(posStart) {
	}
	...
	,startPlayback: function(posStart,posStop) {
		if(posStop == null) posStop = 1;
		if(posStart == null) posStart = 0;
		console.log("PLAAYY");
		this.sound.play();
	}
	,setVolume: function(channelNr,volume) {
	}
	,setVolumes: function(newVolumes) {
	}
	,load: function(url) {
		url += "?ext=.mp3";
		this.sound = eval("new Howl({urls: [\"" + url + "\"]});");
	...
	,posCurrent: null
	,playCallback: null
	,__class__: scorx.audio.js.Mixer
}
scorx.controller = {}
...
scorx.controller.LoadPagesCommand.prototype = $extend(mmvc.impl.Command.prototype,{
	execute: function() {
		...
		this.loader.onPageLoaded = function(pageNr,nrOfPages,data,type) {
			if(pageNr == 0) _g.loadPages.status.dispatch(scorx.controller.LoadPagesStatus.started(nrOfPages)); else _g.loadPages.status.dispatch(scorx.controller.LoadPagesStatus.progress({ pageNr : pageNr, nrOfPages : nrOfPages, data : data}));
			if(pageNr == nrOfPages) _g.loadPages.status.dispatch(scorx.controller.LoadPagesStatus.completed(nrOfPages));
		};
...
scorx.data = {}
scorx.data.ChannelsLoader = function() {
	var _g = this;
	this.result = new msignal.Signal1();
	this.countLoader = new flash.net.URLLoader();
	this.countLoader.set_dataFormat(flash.net.URLLoaderDataFormat.TEXT);
	this.countLoader.addEventListener(flash.events.Event.COMPLETE,$bind(this,this.countComplete));
	this.countLoader.addEventListener(flash.events.IOErrorEvent.IO_ERROR,function(e) {
		_g.result.dispatch(scorx.data.ChannelsResult.error("ChannelsLoader Count IO error",_g.infoUrl));
		console.log("IOERROR");
	});
};
$hxClasses["scorx.data.ChannelsLoader"] = scorx.data.ChannelsLoader;
scorx.data.ChannelsLoader.__name__ = ["scorx","data","ChannelsLoader"];
scorx.data.ChannelsLoader.prototype = {
	onDataResult: function(result) {
		var $e = (result);
		switch( $e[1] ) {
		case 0:
			var tag = $e[3], data = $e[2];
			this.result.dispatch(scorx.data.ChannelsResult.loaded(data,tag));
			this.channelsData.push({ id : tag, label : "XYZ", data : data});
			if(this.channelsData.length == this.nrOfChannels) {
				this.channelsData.sort(function(a,b) {
					return Reflect.compare(a.id,b.id);
				});
				this.result.dispatch(scorx.data.ChannelsResult.complete(this.channelsData));
			}
			break;
		case 2:
			var url = $e[3], message = $e[2];
			console.log(message);
			this.result.dispatch(scorx.data.ChannelsResult.error(message,url));
			break;
		case 1:
			var pageNr = $e[4], bytesTotal = $e[3], bytesLoaded = $e[2];
			break;
		}
	}
	,channels: null
	...
	,loadChannels: function(channels) {
		...
		while(_g < channels.length) {
			...
			console.log(url);
			var dataLoader = new scorx.data.DataLoader();
			dataLoader.result.add($bind(this,this.onDataResult));
			dataLoader.load(url,channel.Id);
		}
	}
	,countComplete: function(e) {
		console.log("complete");
		var urlLoader = js.Boot.__cast(e.target , flash.net.URLLoader);
		var data = urlLoader.data;
		this.channels = null;
		try {
			this.channels = haxe.Json.parse(data);
			console.log(this.channels);
		} catch( e1 ) {
			this.result.dispatch(scorx.data.ChannelsResult.error("ChannelsLoader Count data format error: " + Std.string(e1),this.infoUrl));
			return;
		}
		if(this.channels.length < 1) {
			this.result.dispatch(scorx.data.ChannelsResult.error("ChannelsLoader Count Nr of channels error: Nr of channels =" + Std.string(this.channels.length),this.infoUrl));
			return;
		}
		this.channels.sort(function(a,b) {
			return Reflect.compare(a.Id,b.Id);
		});
		this.result.dispatch(scorx.data.ChannelsResult.started(this.channels));
		console.log("ChannelsLoader JS");
		this.result.dispatch(scorx.data.ChannelsResult.complete(null));
	}
	,load: function(host,productId,userId) {
		console.log("load Channels");
		this.host = host;
		this.productId = productId;
		this.userId = userId;
		this.infoUrl = "" + host + "media/channels/" + productId;
		this.countLoader.load(new flash.net.URLRequest(this.infoUrl));
	}
	,labels: null
	,nrOfLoaded: null
	,nrOfChannels: null
	...
	,countLoader: null
	,result: null
	,__class__: scorx.data.ChannelsLoader
}
scorx.data.ChannelsResult = $hxClasses["scorx.data.ChannelsResult"] = { __ename__ : true, __constructs__ : ["started","loaded","complete","error"] }
scorx.data.ChannelsResult.started = function(channels) { var $x = ["started",0,channels]; $x.__enum__ = scorx.data.ChannelsResult; $x.toString = $estr; return $x; }
scorx.data.ChannelsResult.loaded = function(data,tag) { var $x = ["loaded",1,data,tag]; $x.__enum__ = scorx.data.ChannelsResult; $x.toString = $estr; return $x; }
scorx.data.ChannelsResult.complete = function(channelsData) { var $x = ["complete",2,channelsData]; $x.__enum__ = scorx.data.ChannelsResult; $x.toString = $estr; return $x; }
scorx.data.ChannelsResult.error = function(message,url) { var $x = ["error",3,message,url]; $x.__enum__ = scorx.data.ChannelsResult; $x.toString = $estr; return $x; }
scorx.data.DataLoader = function() {
	this.result = new msignal.Signal1();
	this.loader = new flash.net.URLLoader();
	this.loader.set_dataFormat(flash.net.URLLoaderDataFormat.BINARY);
	this.loader.addEventListener(flash.events.ProgressEvent.PROGRESS,$bind(this,this.onProgress));
	this.loader.addEventListener(flash.events.Event.COMPLETE,$bind(this,this.onComplete));
	this.loader.addEventListener(flash.events.IOErrorEvent.IO_ERROR,$bind(this,this.onIOError));
};
$hxClasses["scorx.data.DataLoader"] = scorx.data.DataLoader;
scorx.data.DataLoader.__name__ = ["scorx","data","DataLoader"];
scorx.data.DataLoader.prototype = {
	...
	,onProgress: function(e) {
		this.result.dispatch(scorx.data.DataResult.progress(e.bytesLoaded,e.bytesTotal,this.tag));
	}
	,onIOError: function(e) {
		this.result.dispatch(scorx.data.DataResult.error("DataLoader IO Error " + Std.string(e),this.url));
	}
	,onComplete: function(e) {
		var data = this.loader.data;
		this.result.dispatch(scorx.data.DataResult.data(data,this.tag));
	}
	,tag: null
	,loader: null
	,url: null
	,result: null
	,__class__: scorx.data.DataLoader
}
scorx.data.DataResult = $hxClasses["scorx.data.DataResult"] = { __ename__ : true, __constructs__ : ["data","progress","error"] }
scorx.data.DataResult.data = function(data,tag) { var $x = ["data",0,data,tag]; $x.__enum__ = scorx.data.DataResult; $x.toString = $estr; return $x; }
scorx.data.DataResult.progress = function(bytesLoaded,bytesTotal,tag) { var $x = ["progress",1,bytesLoaded,bytesTotal,tag]; $x.__enum__ = scorx.data.DataResult; $x.toString = $estr; return $x; }
scorx.data.DataResult.error = function(message,url) { var $x = ["error",2,message,url]; $x.__enum__ = scorx.data.DataResult; $x.toString = $estr; return $x; }
scorx.data.Errors = function() {
	this.errors = new Array();
	this.status = new msignal.Signal1();
};
$hxClasses["scorx.data.Errors"] = scorx.data.Errors;
scorx.data.Errors.__name__ = ["scorx","data","Errors"];
scorx.data.Errors.prototype = {
	clear: function() {
		console.log("clear");
		this.errors = [];
		this.status.dispatch(scorx.data.ErrorStatus.None);
	}
	,addError: function(msg) {
		console.log("add");
		this.errors.push(msg);
		this.status.dispatch(scorx.data.ErrorStatus.Added(msg,this.errors));
	}
	,errors: null
	,status: null
	,__class__: scorx.data.Errors
}
scorx.data.ErrorStatus = $hxClasses["scorx.data.ErrorStatus"] = { __ename__ : true, __constructs__ : ["None","Added"] }
scorx.data.ErrorStatus.None = ["None",0];
scorx.data.ErrorStatus.None.toString = $estr;
scorx.data.ErrorStatus.None.__enum__ = scorx.data.ErrorStatus;
scorx.data.ErrorStatus.Added = function(message,messages) { var $x = ["Added",1,message,messages]; $x.__enum__ = scorx.data.ErrorStatus; $x.toString = $estr; return $x; }
scorx.data.GridLoader = function() {
	var _g = this;
	this.result = new msignal.Signal1();
	this.urlLoader = new flash.net.URLLoader();
	this.urlLoader.set_dataFormat(flash.net.URLLoaderDataFormat.TEXT);
	this.urlLoader.addEventListener(flash.events.Event.COMPLETE,function(e) {
		console.log("complete");
		var urlLoader = js.Boot.__cast(e.target , flash.net.URLLoader);
		var xmlString = urlLoader.data;
		console.log(xmlString);
		try {
			var xml = Xml.parse(xmlString);
		} catch( e1 ) {
			console.log("XML error! " + Std.string(e1));
			_g.result.dispatch(scorx.data.GridResult.error("Grid XML error",_g.url));
			return;
		}
		_g.result.dispatch(scorx.data.GridResult.success(xmlString));
	});
	this.urlLoader.addEventListener(flash.events.IOErrorEvent.IO_ERROR,function(e) {
		console.log("Load error " + _g.url);
		_g.result.dispatch(scorx.data.GridResult.error("Grid Loader IO error",_g.url));
	});
};
$hxClasses["scorx.data.GridLoader"] = scorx.data.GridLoader;
scorx.data.GridLoader.__name__ = ["scorx","data","GridLoader"];
scorx.data.GridLoader.prototype = {
	load: function(host,productId,userId) {
		this.host = host;
		this.productId = productId;
		this.userId = userId;
	...
	,url: null
	,urlLoader: null
	,__class__: scorx.data.GridLoader
}
scorx.data.GridResult = $hxClasses["scorx.data.GridResult"] = { __ename__ : true, __constructs__ : ["success","error"] }
scorx.data.GridResult.success = function(xmlString) { var $x = ["success",0,xmlString]; $x.__enum__ = scorx.data.GridResult; $x.toString = $estr; return $x; }
scorx.data.GridResult.error = function(message,url) { var $x = ["error",1,message,url]; $x.__enum__ = scorx.data.GridResult; $x.toString = $estr; return $x; }
scorx.data.GridProc = function() {
	console.log("New GridProc");
};
$hxClasses["scorx.data.GridProc"] = scorx.data.GridProc;
scorx.data.GridProc.__name__ = ["scorx","data","GridProc"];
scorx.data.GridProc.prototype = {
	testGridSystems: function() {
		var gridSystems = { systems : new Array()};
		var gridSystem = { pos : 0.1, pageNr : 0, x : 0.2, y : 0.2, width : 0.7, height : 0.3, bars : []};
		gridSystems.systems.push(gridSystem);
		var gridBar = { pos : 0.2, x : 0.35};
		gridSystem.bars.push(gridBar);
		var gridBar1 = { pos : 0.3, x : 0.5};
		gridSystem.bars.push(gridBar1);
		var gridSystem1 = { pos : 0.5, pageNr : 0, x : 0.2, y : 0.6, width : 0.6, height : 0.3, bars : []};
		gridSystems.systems.push(gridSystem1);
		var gridSystem2 = { pos : 0.8, pageNr : 1, x : 0.2, y : 0.3, width : 0.7, height : 0.4, bars : []};
		gridSystems.systems.push(gridSystem2);
		var gridBar2 = { pos : 0.9, x : 0.6};
		gridSystem2.bars.push(gridBar2);
		return gridSystems;
	}
	,testXml: function() {
	...
	,xmlToGridSystems: function(xmlString) {
		...
		while( items.hasNext() ) {
			...
			var pos = this.getFloat(item.get("pos"));
			var pageNr = Std.parseInt(item.get("page"));
			var x = this.getFloat(item.get("x"));
			var y = this.getFloat(item.get("y"));
			var w = this.getFloat(item.get("w"));
			var h = this.getFloat(item.get("h"));
			console.log(item.get("pos"));
			var _g = item.get("type");
			switch(_g) {
			case "system":
	...
	,findPageBarFromPos: function(currentPos) {
		var count = this.pageBars.length;
		var _g = 0;
		while(_g < count) {
			var i = _g++;
			var pageBar = this.pageBars[count - 1 - i];
			if(currentPos > pageBar.pos) {
				var span = (currentPos - pageBar.pos) / (pageBar.nextPos - pageBar.pos);
				pageBar.deltaX = pageBar.rect.x + span * pageBar.rect.width;
				return pageBar;
			}
		}
		return { pageIdx : this.pageBars[0].pageIdx, barIdx : 0, sysIdx : 0, pos : 0, nextPos : this.pageBars[0].pos, rect : this.pageBars[0].rect, deltaX : this.pageBars[0].rect.x};
	}
	,updatePointerPosition: function(pageBar) {
	}
	,setPointerPosition: function(value) {
		var pageBar = this.findPageBarFromPos(value);
		this.updatePointerPosition(pageBar);
	}
	,getPositionFromCoords: function(x,y) {
		var _g = 0, _g1 = this.pageBars;
		while(_g < _g1.length) {
			var pageBar = _g1[_g];
			++_g;
			if(pageBar.rect.contains(x,y)) {
				var delta = (x - pageBar.rect.x) / pageBar.rect.width;
				return pageBar.pos + delta * (pageBar.nextPos - pageBar.pos);
			}
		}
		return -1;
	}
	,getPageBars: function(docInfo) {
		if(docInfo.pageInfos == null) return null;
		if(docInfo.pageInfos.length < 1) return null;
		this.pageBars = [];
		this.pageCoordinates = new haxe.ds.IntMap();
		this.sysCoordinates = new haxe.ds.ObjectMap();
		var systemIdx = 0;
		var barIdx = 0;
		var _g = 0, _g1 = this.gridSystems.systems;
		while(_g < _g1.length) {
			var gridSystem = _g1[_g];
			++_g;
			var nextSystemPos = -1;
			nextSystemPos = systemIdx == this.gridSystems.systems.length - 1?1:this.gridSystems.systems[systemIdx + 1].pos;
			var pageNr = gridSystem.pageNr;
			...
			var systemRect = new flash.geom.Rectangle(pageRect.x + gridSystem.x * pageWidth,pageRect.y + gridSystem.y * pageHeight,gridSystem.width * pageWidth,gridSystem.height * pageHeight);
			if(gridSystem.bars == null || gridSystem.bars.length == 0) {
				var pageBar = { pageIdx : gridSystem.pageNr, barIdx : barIdx, sysIdx : systemIdx, pos : gridSystem.pos, nextPos : nextSystemPos, rect : systemRect, deltaX : -1};
				this.pageBars.push(pageBar);
				barIdx++;
				this.pageCoordinates.set(pageBar.pageIdx,pageRect);
				this.sysCoordinates.set(pageBar,systemRect);
			} else {
				var barRect = systemRect.clone();
				barRect.width = gridSystem.bars[0].x * pageWidth;
				var pageBar = { pageIdx : gridSystem.pageNr, barIdx : barIdx, sysIdx : systemIdx, pos : gridSystem.pos, nextPos : gridSystem.bars[0].pos, rect : barRect, deltaX : -1};
				this.pageBars.push(pageBar);
				barIdx++;
				this.pageCoordinates.set(pageBar.pageIdx,pageRect);
				this.sysCoordinates.set(pageBar,systemRect);
				var _g3 = 0, _g2 = gridSystem.bars.length;
				while(_g3 < _g2) {
					var i = _g3++;
					var gridBar = gridSystem.bars[i];
					var barX = systemRect.x + gridBar.x * pageWidth;
					var barX2 = 0;
					var nextPos = 0;
					if(i < gridSystem.bars.length - 1) {
						var nextGridBar = gridSystem.bars[i + 1];
					...
					var barRect1 = new flash.geom.Rectangle(barX,systemRect.y,barX2 - barX,systemRect.height);
					var pageBar1 = { pageIdx : gridSystem.pageNr, barIdx : barIdx, sysIdx : systemIdx, pos : gridBar.pos, nextPos : nextPos, rect : barRect1, deltaX : -1};
					this.pageBars.push(pageBar1);
					barIdx++;
					this.pageCoordinates.set(pageBar1.pageIdx,pageRect);
					this.sysCoordinates.set(pageBar1,systemRect);
				}
			}
			systemIdx++;
		}
	...
	,debugGridSystems: function(gridSystems) {
		var _g = 0, _g1 = gridSystems.systems;
		while(_g < _g1.length) {
			var gridSystem = _g1[_g];
			++_g;
			var _g2 = 0, _g3 = gridSystem.bars;
			while(_g2 < _g3.length) {
				var gridBar = _g3[_g2];
				++_g2;
			}
	...
	,afterInit: function(gridSystems) {
	...
	,pageBars: null
	,gridSystems: null
	,__class__: scorx.data.GridProc
}
scorx.data.ImgLoader = function() {
	this.result = new msignal.Signal1();
	this.loader = new flash.display.Loader();
	this.loaderInfo = this.loader.contentLoaderInfo;
	this.loaderInfo.addEventListener(flash.events.ProgressEvent.PROGRESS,$bind(this,this.onProgress));
	this.loaderInfo.addEventListener(flash.events.Event.COMPLETE,$bind(this,this.onComplete));
	this.loaderInfo.addEventListener(flash.events.IOErrorEvent.IO_ERROR,$bind(this,this.onIOError));
};
$hxClasses["scorx.data.ImgLoader"] = scorx.data.ImgLoader;
scorx.data.ImgLoader.__name__ = ["scorx","data","ImgLoader"];
scorx.data.ImgLoader.prototype = {
	load: function(url,pageNr) {
	...
	,onProgress: function(e) {
		this.result.dispatch(scorx.data.ImgResult.progress(e.bytesLoaded,e.bytesTotal,this.pageNr));
	}
	,onIOError: function(e) {
		this.result.dispatch(scorx.data.ImgResult.error("ImgLoader IO Error " + Std.string(e),this.url));
	}
	,onComplete: function(e) {
		var loaderInfo = js.Boot.__cast(e.target , flash.display.LoaderInfo);
		var data = (js.Boot.__cast(this.loader.content , flash.display.Bitmap)).bitmapData;
		this.result.dispatch(scorx.data.ImgResult.data(data,this.pageNr));
	}
	,pageNr: null
	,loader: null
	,loaderInfo: null
	,url: null
	,result: null
	,__class__: scorx.data.ImgLoader
}
scorx.data.ImgResult = $hxClasses["scorx.data.ImgResult"] = { __ename__ : true, __constructs__ : ["data","progress","error"] }
scorx.data.ImgResult.data = function(data,pageNr) { var $x = ["data",0,data,pageNr]; $x.__enum__ = scorx.data.ImgResult; $x.toString = $estr; return $x; }
scorx.data.ImgResult.progress = function(bytesLoaded,bytesTotal,pageNr) { var $x = ["progress",1,bytesLoaded,bytesTotal,pageNr]; $x.__enum__ = scorx.data.ImgResult; $x.toString = $estr; return $x; }
scorx.data.ImgResult.error = function(message,url) { var $x = ["error",2,message,url]; $x.__enum__ = scorx.data.ImgResult; $x.toString = $estr; return $x; }
scorx.data.PagesLoader = function() {
	var _g = this;
	this.result = new msignal.Signal1();
	this.countLoader = new flash.net.URLLoader();
	this.countLoader.set_dataFormat(flash.net.URLLoaderDataFormat.TEXT);
	this.countLoader.addEventListener(flash.events.Event.COMPLETE,$bind(this,this.countComplete));
	this.countLoader.addEventListener(flash.events.IOErrorEvent.IO_ERROR,function(e) {
		_g.result.dispatch(scorx.data.PagesResult.error("ScreenLoader Count IO error",_g.url));
		console.log("IOERROR");
	});
};
$hxClasses["scorx.data.PagesLoader"] = scorx.data.PagesLoader;
scorx.data.PagesLoader.__name__ = ["scorx","data","PagesLoader"];
scorx.data.PagesLoader.prototype = {
	onImgLoaderResult: function(result) {
		var $e = (result);
		switch( $e[1] ) {
		case 0:
			var pageNr = $e[3], data = $e[2];
			this.result.dispatch(scorx.data.PagesResult.success(pageNr,this.nrOfPages,data));
			this.nrOfLoaded++;
			if(this.nrOfLoaded == this.nrOfPages) this.result.dispatch(scorx.data.PagesResult.complete(this.nrOfPages));
			break;
		case 2:
			var url = $e[3], message = $e[2];
			this.result.dispatch(scorx.data.PagesResult.error(message,url));
			break;
		case 1:
			var pageNr = $e[4], bytesTotal = $e[3], bytesLoaded = $e[2];
			break;
	...
	,loadPages: function(nrOfPages) {
		...
		while(_g < nrOfPages) {
			...
			console.log(url);
			var imgLoader = new scorx.data.ImgLoader();
			imgLoader.result.add($bind(this,this.onImgLoaderResult));
	...
	,load: function(host,productId,userId) {
		...
		this.countLoader.load(new flash.net.URLRequest(this.countUrl));
	}
	,countComplete: function(e) {
		console.log("complete");
		var urlLoader = js.Boot.__cast(e.target , flash.net.URLLoader);
		...
		try {
			...
			this.result.dispatch(scorx.data.PagesResult.error("ScreenLoader Count data format error: " + Std.string(e1),this.countUrl));
			return;
		}
		if(nrOfPages < 1) {
			this.result.dispatch(scorx.data.PagesResult.error("ScreenLoader Count Nr of pages error: Nr of pages =" + Std.string(nrOfPages),this.countUrl));
			return;
		}
		this.result.dispatch(scorx.data.PagesResult.started(nrOfPages));
	...
	,countLoader: null
	,result: null
	,__class__: scorx.data.PagesLoader
}
scorx.data.PagesResult = $hxClasses["scorx.data.PagesResult"] = { __ename__ : true, __constructs__ : ["started","success","complete","error"] }
scorx.data.PagesResult.started = function(nrOfPages) { var $x = ["started",0,nrOfPages]; $x.__enum__ = scorx.data.PagesResult; $x.toString = $estr; return $x; }
scorx.data.PagesResult.success = function(pageNr,nrOfPages,data) { var $x = ["success",1,pageNr,nrOfPages,data]; $x.__enum__ = scorx.data.PagesResult; $x.toString = $estr; return $x; }
scorx.data.PagesResult.complete = function(nrOfPages) { var $x = ["complete",2,nrOfPages]; $x.__enum__ = scorx.data.PagesResult; $x.toString = $estr; return $x; }
scorx.data.PagesResult.error = function(message,url) { var $x = ["error",3,message,url]; $x.__enum__ = scorx.data.PagesResult; $x.toString = $estr; return $x; }
scorx.data.Zooma = function() {
	this.zoomSignal = new msignal.Signal1();
	this.zoom = sx.mvc.view.enums.ScrollWidgetZoom.Zoom60;
};
$hxClasses["scorx.data.Zooma"] = scorx.data.Zooma;
scorx.data.Zooma.__name__ = ["scorx","data","Zooma"];
scorx.data.Zooma.prototype = {
	getZoom: function() {
		return this.zoom;
	}
...
scorx.model.AccessLevelView = $hxClasses["scorx.model.AccessLevelView"] = { __ename__ : true, __constructs__ : ["SampleView","OrganizationView","FullView"] }
scorx.model.AccessLevelView.SampleView = ["SampleView",0];
scorx.model.AccessLevelView.SampleView.toString = $estr;
scorx.model.AccessLevelView.SampleView.__enum__ = scorx.model.AccessLevelView;
scorx.model.AccessLevelView.OrganizationView = ["OrganizationView",1];
scorx.model.AccessLevelView.OrganizationView.toString = $estr;
scorx.model.AccessLevelView.OrganizationView.__enum__ = scorx.model.AccessLevelView;
scorx.model.AccessLevelView.FullView = ["FullView",2];
scorx.model.AccessLevelView.FullView.toString = $estr;
scorx.model.AccessLevelView.FullView.__enum__ = scorx.model.AccessLevelView;
...
scorx.model.Configuration.prototype = {
	getScreenCountUrl: function(_productId,_userId) {
		if(_userId == null) _userId = 0;
		if(_productId == null) _productId = 0;
		if(_productId == 0) _productId = this.productId;
		if(_userId == 0) _userId = this.userId;
		var url = "" + this.host + scorx.model.Configuration.MEDIA_SCREEN_COUNT + "/" + _productId;
		return url;
	}
	,getGridUrl: function(_productId,_userId) {
		if(_userId == null) _userId = 0;
		if(_productId == null) _productId = 0;
		if(_productId == 0) _productId = this.productId;
		if(_userId == 0) _userId = this.userId;
		var url = "" + this.host + scorx.model.Configuration.MEDIA_GRID + "/" + _productId + "/" + _userId;
		return url;
	}
	,updated: null
	,setValues: function(productId,userId,host,playbackLevel,viewLevel) {
		...
		this.viewLevel = viewLevel == null?scorx.model.AccessLevelView.OrganizationView:cx.EnumTools.createFromString(scorx.model.AccessLevelView,viewLevel);
...
scorx.model.PlayPositionInfo = function(pos,sender,pageIdx) {
	this.pos = pos;
	this.sender = sender;
	this.pageIdx = pageIdx;
};
$hxClasses["scorx.model.PlayPositionInfo"] = scorx.model.PlayPositionInfo;
scorx.model.PlayPositionInfo.__name__ = ["scorx","model","PlayPositionInfo"];
scorx.model.PlayPositionInfo.prototype = {
	pageIdx: null
	,sender: null
	,pos: null
	,__class__: scorx.model.PlayPositionInfo
}
scorx.model.PlayPosition = function() {
	this.prevPos = 0;
	this.currentPos = 0;
	this.prevBarIdx = 0;
	this.prevSysIdx = 0;
	this.prevPageIdx = 0;
	var _g = this;
	this.positionSignal = new msignal.Signal1();
	cx.TimerTools.timer(function() {
		if(_g.pageBar == null) return;
		if(_g.currentPos != _g.prevPos) _g.positionSignal.dispatch(new scorx.model.PlayPositionInfo(_g.currentPos,_g.sender,_g.pageBar.pageIdx));
		_g.prevPos = _g.currentPos;
	},500);
};
$hxClasses["scorx.model.PlayPosition"] = scorx.model.PlayPosition;
scorx.model.PlayPosition.__name__ = ["scorx","model","PlayPosition"];
scorx.model.PlayPosition.prototype = {
	viewportCanContainsRects: function(vPort,rect1,rect2) {
		var rect3 = rect1.union(rect2);
		console.log(vPort);
		console.log(rect3);
		return vPort.containsRect(rect3);
	}
	,setViewPort: function(viewPort,scrollX,scrollY) {
	...
	,checkScroll: function() {
		...
		var nextBar = this.pageBar.barIdx < this.gridProc.pageBars.length?this.gridProc.pageBars[this.pageBar.barIdx + 1]:this.pageBar;
		var vPort = this.viewPort.clone();
		var thisBarRect = this.pageBar.rect.clone();
		thisBarRect.offset(this.scrollX,this.scrollY);
		console.log(thisBarRect);
		if(!vPort.containsRect(thisBarRect)) {
			if(thisBarRect.y < vPort.y) {
				console.log("Kan inte se toppen");
				this.scroll(0,thisBarRect.y);
			}
			if(thisBarRect.y + thisBarRect.height > vPort.y + vPort.height) console.log("Kan inte se botten");
		}
	}
	,getPosition: function() {
		return this.currentPos;
	}
	,setPosition: function(currentPos,sender) {
		if(this.gridProc.pageBars == null) return;
		this.sender = sender;
		this.currentPos = currentPos;
		this.pageBar = this.gridProc.findPageBarFromPos(currentPos);
		this.pageSystemUtils.updatePointerPosition(this.pageBar.deltaX,this.pageBar.rect.y,this.pageBar.rect.height);
	}
	,prevPos: null
	,currentPos: null
	,pageBar: null
	...
	,sender: null
	,positionSignal: null
	,pageSystemUtils: null
	,gridProc: null
	,__class__: scorx.model.PlayPosition
}
...
scorx.model.PlaybackEngine = function() {
	this.channelIds = [];
	this.onStart = new msignal.Signal1();
	this.onStop = new msignal.Signal0();
	this.onPosition = new msignal.Signal1();
	this.onError = new msignal.Signal1();
};
$hxClasses["scorx.model.PlaybackEngine"] = scorx.model.PlaybackEngine;
scorx.model.PlaybackEngine.__name__ = ["scorx","model","PlaybackEngine"];
scorx.model.PlaybackEngine.prototype = {
	mixerError: function(msg) {
		console.log(msg);
		this.onError.dispatch(msg);
	}
	,mixerStop: function() {
		console.log("MIXER mixerStop");
		this.onStop.dispatch();
	}
	,mixerStart: function(pos) {
		console.log("MIXER mixerStart");
		this.onStart.dispatch(pos);
	}
	,setVolume: function(channelId,value) {
		var chNr = Lambda.indexOf(this.channelIds,channelId);
	...
	,getSoundLength: function() {
		return this.sounds[0].length;
	}
	,loadOneFile: function(url) {
		var _g = this;
		this.mixer = new scorx.audio.js.Mixer(function(startPos,endPos,curPos) {
			_g.onPosition.dispatch(curPos);
		});
		this.mixer.load(url);
	}
	...
	,mixer: null
	,sounds: null
	,__class__: scorx.model.PlaybackEngine
}
...
scorx.types = {}
scorx.types.ScoreLoadingType = $hxClasses["scorx.types.ScoreLoadingType"] = { __ename__ : true, __constructs__ : ["screen","print","thumb"] }
scorx.types.ScoreLoadingType.screen = ["screen",0];
scorx.types.ScoreLoadingType.screen.toString = $estr;
scorx.types.ScoreLoadingType.screen.__enum__ = scorx.types.ScoreLoadingType;
scorx.types.ScoreLoadingType.print = ["print",1];
scorx.types.ScoreLoadingType.print.toString = $estr;
scorx.types.ScoreLoadingType.print.__enum__ = scorx.types.ScoreLoadingType;
scorx.types.ScoreLoadingType.thumb = ["thumb",2];
scorx.types.ScoreLoadingType.thumb.toString = $estr;
scorx.types.ScoreLoadingType.thumb.__enum__ = scorx.types.ScoreLoadingType;
scorx.ui = {}
scorx.ui.ChannelWidget = function(channelLabel,channelId) {
	ru.stablex.ui.widgets.Widget.call(this);
	this.channelId = channelId;
	this.set_w(80);
	this.set_h(30);
	this.label = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Text);
	this.label.set_text(channelLabel);
	this.label.format = new flash.text.TextFormat("Arial",11,0,true);
	this.label.set_w(80);
	this.label.set_h(12);
	this.addChild(this.label);
	this.progress = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Progress);
	this.progress.defaults = "Default";
	this.progress.set_interactive(true);
	this.progress.set_w(80);
	this.progress.set_h(16);
	this.progress.set_y(20);
	this.progress.set_max(1);
	this.progress.set_value(0.7);
	this.addChild(this.progress);
	this.progress.addEventListener("widgetChange",$bind(this,this.onChange));
	this.progress.set_useHandCursor(true);
};
$hxClasses["scorx.ui.ChannelWidget"] = scorx.ui.ChannelWidget;
scorx.ui.ChannelWidget.__name__ = ["scorx","ui","ChannelWidget"];
scorx.ui.ChannelWidget.__super__ = ru.stablex.ui.widgets.Widget;
scorx.ui.ChannelWidget.prototype = $extend(ru.stablex.ui.widgets.Widget.prototype,{
	onChangeValue: function(channelId,value) {
	}
	,onChange: function(e) {
		this.onChangeValue(this.channelId,this.progress.get_value());
	}
	,channelId: null
	,label: null
	,progress: null
	,__class__: scorx.ui.ChannelWidget
});
scorx.view = {}
...
scorx.view.ButtonsMediator.prototype = $extend(mmvc.impl.Mediator.prototype,{
	onRegister: function() {
		...
		this.view.btnLoad.addEventListener(flash.events.MouseEvent.MOUSE_DOWN,function(e) {
			...
			loadParameters.type = scorx.types.ScoreLoadingType.screen;
...
sx.mvc.view.ScrollWidgetView = function() {
	...
	this.pagesOverlay = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Widget);
	this.widget.addChild(this.pagesOverlay);
	this.doubleClickEnabled = true;
	this.pagesOverlay.doubleClickEnabled = true;
	this.pagesOverlay.addEventListener(flash.events.MouseEvent.DOUBLE_CLICK,function(e) {
		console.log("double");
		_g.handleMouseDoubleClick(_g.pagesOverlay.get_mouseX(),_g.pagesOverlay.get_mouseY());
	});
	this.gridOverlay = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Widget);
	this.widget.addChild(this.gridOverlay);
	this.orgOverlay = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Widget);
...
sx.mvc.view.ScrollWidgetView.prototype = $extend(ru.stablex.ui.widgets.Scroll.prototype,{
	...
	,clearPages: function() {
		while(this.pagesOverlay.nmeChildren.length > 0) this.pagesOverlay.removeChildAt(0);
	}
	,initPages: function(nrOfPages) {
		...
		while(_g < nrOfPages) {
			var i = _g++;
			this.pagesOverlay.addChild(this.getTest());
		}
	...
	,afterReziseGrid: function(gridOverlay,docInfo) {
		console.log("AFTER RESIZE GRID");
	}
	,resize_: function(x,y,width,height) {
		...
		var docInfo = cx.flash.layout.DocumentLayout.arrange(this.pagesOverlay,width,size.x,size.y);
		this.widget.set_w(docInfo.holderRect.width);
		this.widget.set_h(docInfo.holderRect.height);
		this.pageSize = docInfo.pageSize;
		this.gridOverlay.set_x(this.pagesOverlay.get_x());
		this.gridOverlay.set_w(this.pagesOverlay.get_w());
		this.gridOverlay.set_h(this.pagesOverlay.get_h());
		this.orgOverlay.set_x(this.pagesOverlay.get_x());
		this.orgOverlay.set_w(this.pagesOverlay.get_w());
		this.orgOverlay.set_h(this.pagesOverlay.get_h());
...
scorx.view.PagesMediator.prototype = $extend(mmvc.impl.Mediator.prototype,{
	redrawGrid: function(gridOverlay,docInfo) {
		if(this.gridProc.gridSystems == null) return;
		if(this.docInfo.pageInfos.length < 1) return;
		this.pageBars = this.gridProc.getPageBars(docInfo);
		this.pageSystemUtils.drawPageBars(this.pageBars);
		this.playPosition.setViewPort(new flash.geom.Rectangle(0,0,this.view.get_w(),this.view.get_h()),this.view.get_scrollX(),this.view.get_scrollY());
		if(this.configuration.viewLevel == scorx.model.AccessLevelView.OrganizationView) this.pageSystemUtils.drawOrgMasks(this.pageBars,-1,this.gridProc.pageCoordinates);
		this.playPosition.setPosition(this.playPosition.getPosition(),this.view);
	}
	,onRegister: function() {
		var _g = this;
		this.view.setZoom(this.zooma.getZoom());
		this.view.addEventListener("widgetScrollStop",function(e) {
			_g.playPosition.setViewPort(new flash.geom.Rectangle(0,0,_g.view.get_w(),_g.view.get_h()),_g.view.get_scrollX(),_g.view.get_scrollY());
		});
		this.mediate(this.pagesLoader.result.add(function(result) {
			var $e = (result);
			switch( $e[1] ) {
			case 0:
				...
				break;
			case 1:
				var data = $e[4], nrOfPages = $e[3], pageNr = $e[2];
				_g.view.addPage(pageNr,nrOfPages,data);
				break;
			case 2:
				var nrOfPages = $e[2];
				_g.view.afterResize(0,0,0,0);
				break;
			case 3:
				var url = $e[3], message = $e[2];
		...
		this.gridLoader.result.add(function(result) {
			...
			case 0:
				var xmlString = $e[2];
				try {
					_g.gridProc.init(xmlString);
				} catch( e ) {
					_g.errors.addError("Grid error - " + xmlString);
				}
				break;
			case 1:
				var url = $e[3], message = $e[2];
		...
		this.mediate(this.playPosition.positionSignal.add(function(info) {
			if(_g.pageBars == null) return;
			if(_g.configuration.viewLevel == scorx.model.AccessLevelView.OrganizationView) {
				var pageIdx = info.pageIdx;
				if(info.pos == 0) pageIdx = -1;
				if(info.pos >= 1) pageIdx = -1;
				_g.pageSystemUtils.drawOrgMasks(_g.pageBars,pageIdx,_g.gridProc.pageCoordinates);
			}
		}));
		this.zooma.zoomSignal.add(function(zoom) {
			_g.view.setZoom(zoom);
		});
		this.view.afterReziseGrid = function(gridOverlay,docInfo) {
			console.log("afterReziseGrid");
			_g.docInfo = docInfo;
			_g.redrawGrid(_g.view.gridOverlay,_g.docInfo);
		};
		this.view.handleMouseDoubleClick = function(x,y) {
			console.log([x,y]);
			var pos = _g.gridProc.getPositionFromCoords(x,y);
			if(pos == -1) return;
			_g.playPosition.setPosition(pos,_g.view);
			var oneSecond = 1000 / _g.playbackEngine.getSoundLength();
			_g.playbackEngine.start(pos - oneSecond / 2);
		};
		this.gridProc.afterInit = function(gridSystems) {
			console.log("gridProc.afterInit");
			_g.pageSystemUtils.init(_g.view.gridOverlay,_g.view.orgOverlay);
			_g.playPosition.setViewPort(new flash.geom.Rectangle(0,0,_g.view.get_w(),_g.view.get_h()),_g.view.get_scrollX(),_g.view.get_scrollY());
			_g.redrawGrid(_g.view.gridOverlay,_g.docInfo);
		};
	}
...
sx.data.ScoreLoader.prototype = {
	...
	,loadPages: function(type) {
		if(type == null) type = scorx.types.ScoreLoadingType.screen;
...
sx.mvc.app.AppView.__name__ = ["sx","mvc","app","AppView"];
sx.mvc.app.AppView.__interfaces__ = [mmvc.api.IViewContainer];
sx.mvc.app.AppView.__super__ = flash.display.Sprite;
sx.mvc.app.AppView.prototype = $extend(flash.display.Sprite.prototype,{
	activeMediators: function(view) {
		this.viewAdded(view);
	}
...
$hxClasses["sx.mvc.view.BoxView"] = sx.mvc.view.BoxView;
sx.mvc.view.BoxView.__name__ = ["sx","mvc","view","BoxView"];
sx.mvc.view.BoxView.__super__ = ru.stablex.ui.widgets.Box;
sx.mvc.view.BoxView.prototype = $extend(ru.stablex.ui.widgets.Box.prototype,{
	addSkin: function() {
	}
	,createChildren: function() {
		var label = ru.stablex.ui.UIBuilder.create(ru.stablex.ui.widgets.Text);
		label.set_text("This is BoxView - createChildren() should be overridden!");
		label.set_w(400);
		this.addChild(label);
	}
	,__class__: sx.mvc.view.BoxView
});
sx.mvc.view.enums = {}
...
sx.player = {}
sx.player.grid = {}
sx.player.grid.PageSystemsUtils = function() {
	this.currentNotPageIdx = -2;
};
$hxClasses["sx.player.grid.PageSystemsUtils"] = sx.player.grid.PageSystemsUtils;
sx.player.grid.PageSystemsUtils.__name__ = ["sx","player","grid","PageSystemsUtils"];
sx.player.grid.PageSystemsUtils.prototype = {
	drawOrgMasks: function(pageBars,notPageIdx,pageCoordinates) {
		this.currentNotPageIdx = notPageIdx;
		this.orgTarget.get_graphics().clear();
		this.orgTarget.get_graphics().beginFill(16711680);
		...
		while( keys.hasNext() ) {
			...
			this.orgTarget.get_graphics().drawCircle(pageRect.x + pageRect.width / 2,pageRect.y + pageRect.height / 2,radius);
		}
	}
	,currentNotPageIdx: null
	,updatePointerPosition: function(x,y,height) {
		this.pointer.set_x(x);
		this.pointer.set_y(y);
		this.pointer.set_scaleY(height / 100);
	}
	,createPointer: function(pointerPosition) {
		this.pointer.get_graphics().lineStyle(2,16711680,0.5);
		this.pointer.get_graphics().moveTo(0,0);
		this.pointer.get_graphics().lineTo(0,100);
	}
	,clear: function(target) {
		this.target.get_graphics().clear();
	}
	,drawPageBars: function(pageBars) {
		this.clear(this.target);
		this.target.get_graphics().lineStyle(1,11184810);
		var _g = 0;
		while(_g < pageBars.length) {
			var pageBar = pageBars[_g];
			++_g;
	...
	,init: function(target,orgTarget) {
		this.target = target;
		while(this.target.nmeChildren.length > 0) this.target.removeChildAt(0);
		this.pointer = new flash.display.Sprite();
		this.target.addChild(this.pointer);
		this.createPointer({ x : 10, y : 10, height : 100, pageIdx : 0});
		this.orgTarget = orgTarget;
		this.orgTarget.get_graphics().clear();
		this.orgTarget.get_graphics().beginFill(65280);
	}
	,orgTarget: null
	,pointer: null
	,target: null
	,__class__: sx.player.grid.PageSystemsUtils
}
...
haxe.Resource.content = [{ name : "NME_:bitmap_player_view_TestBitmap", data : "s560:iVBORw0KGgoAAAANSUhEUgAAABUAAAATCAYAAAB:TkaLAAAAAXNSR0IArs4c6QAAAAZiS0dEAP8A:wD:oL2nkwAAAAlwSFlzAAALEwAACxMBAJqcGAAAAAd0SU1FB90CCAwPN%s:nnoAAAAZdEVYdENvbW1lbnQAQ3JlYXRlZCB3aXRoIEdJTVBXgQ4XAAAA:0lEQVQ4y83UMYoCMRTG8b%DWAkeIMdIGmE7YUFyCSvBykVYUBaEAUEQBqytvEQOYGWTHCOtICwIVtuMEsaMMyoLvi4f4cdL8gi8a0njR9L49mXdALDWDoDWo5hSaiONz4AJsAf6TotjM9zwRIerHAToAinwlbxw5CXwHUQ7YAaQvABOC2DfaXF6Cq0CS1Fp:FoaP4:kiyowikrj18AYSKXx0yBPgZ8qEKBZANvARxAtpfFnoAPMY6C1dghslVLnaKdOi1:gE3BBnJWBZfd%c3ynxSEC1wZLHyqHewW4Fnh3pJwWxwCuDVbOaQDXBm9e:w78UCX:8RVeO83n7X3rD%f:bDF3L3uGAAAAAElFTkSuQmCC"}];
var q = window.jQuery;
js.JQuery = q;
...
Config.productId = 109;
Config.userId = 1;
Config.playbackLevel = "FullPlayback";
Config.viewLevel = "OrganizationView";
Config.MANAGER_APPLICATION_ID = "print.mgr.PrintMgr";
Config.MANAGER_PUBLISHER_ID = "";
Config.MANAGER_APPLICATION_URL = "http://dev.korakademin.se/app/PrintMgr.air";
Config.MANAGER_APPLICATION_VERSION = "0.304";
...
cx.layout.ILayoutItem.__meta__ = { obj : { 'interface' : null}};
...
flash.display.IGraphicsFill.__meta__ = { obj : { 'interface' : null}};
flash.display.IGraphicsData.__meta__ = { obj : { 'interface' : null}};
flash.display.IGraphicsPath.__meta__ = { obj : { 'interface' : null}};
...
flash.display.IGraphicsStroke.__meta__ = { obj : { 'interface' : null}};
...
motion.actuators.IGenericActuator.__meta__ = { obj : { 'interface' : null}};
...
player.AppMediator.__meta__ = { fields : { errors : { name : ["errors"], type : ["scorx.data.Errors"], inject : null}, channelsLoader : { name : ["channelsLoader"], type : ["scorx.data.ChannelsLoader"], inject : null}, pagesLoader : { name : ["pagesLoader"], type : ["scorx.data.PagesLoader"], inject : null}, gridLoader : { name : ["gridLoader"], type : ["scorx.data.GridLoader"], inject : null}, config : { name : ["config"], type : ["scorx.model.Configuration"], inject : null}, confload : { name : ["confload"], type : ["scorx.controller.Confload"], inject : null}}};
...
player.view.ErrordisplayMediator.__meta__ = { fields : { errors : { name : ["errors"], type : ["scorx.data.Errors"], inject : null}}};
player.view.PlaybackMixerMediator.__meta__ = { fields : { playPosition : { name : ["playPosition"], type : ["scorx.model.PlayPosition"], inject : null}, channelsLoader : { name : ["channelsLoader"], type : ["scorx.data.ChannelsLoader"], inject : null}, playbackEngine : { name : ["playbackEngine"], type : ["scorx.model.PlaybackEngine"], inject : null}}};
player.view.ScrollerMediator.__meta__ = { fields : { errors : { name : ["errors"], type : ["scorx.data.Errors"], inject : null}, playbackEngine : { name : ["playbackEngine"], type : ["scorx.model.PlaybackEngine"], inject : null}, channelsLoader : { name : ["channelsLoader"], type : ["scorx.data.ChannelsLoader"], inject : null}, pagesLoader : { name : ["pagesLoader"], type : ["scorx.data.PagesLoader"], inject : null}, gridLoader : { name : ["gridLoader"], type : ["scorx.data.GridLoader"], inject : null}, configuration : { name : ["configuration"], type : ["scorx.model.Configuration"], inject : null}, playPosition : { name : ["playPosition"], type : ["scorx.model.PlayPosition"], inject : null}}};
player.view.TestBitmap.resourceName = "NME_:bitmap_player_view_TestBitmap";
player.view.ZoomView.BTN_H = 32;
player.view.ZoomMediator.__meta__ = { fields : { zooma : { name : ["zooma"], type : ["scorx.data.Zooma"], inject : null}}};
...
scorx.controller.LoadPagesCommand.__meta__ = { fields : { loader : { name : ["loader"], type : ["sx.data.ScoreLoader"], inject : null}, loadParameters : { name : ["loadParameters"], type : ["scorx.controller.LoadParameters"], inject : null}, loadPages : { name : ["loadPages"], type : ["scorx.controller.LoadPages"], inject : null}}};
...
scorx.model.Configuration.MEDIA_GRID = "media/grid";
scorx.model.Configuration.MEDIA_SCREEN_COUNT = "media/screen/count";
scorx.model.PlayPosition.__meta__ = { fields : { pageSystemUtils : { name : ["pageSystemUtils"], type : ["sx.player.grid.PageSystemsUtils"], inject : null}, gridProc : { name : ["gridProc"], type : ["scorx.data.GridProc"], inject : null}}};
...
scorx.view.PagesMediator.__meta__ = { fields : { zooma : { name : ["zooma"], type : ["scorx.data.Zooma"], inject : null}, configuration : { name : ["configuration"], type : ["scorx.model.Configuration"], inject : null}, playbackEngine : { name : ["playbackEngine"], type : ["scorx.model.PlaybackEngine"], inject : null}, errors : { name : ["errors"], type : ["scorx.data.Errors"], inject : null}, playPosition : { name : ["playPosition"], type : ["scorx.model.PlayPosition"], inject : null}, pageSystemUtils : { name : ["pageSystemUtils"], type : ["sx.player.grid.PageSystemsUtils"], inject : null}, gridProc : { name : ["gridProc"], type : ["scorx.data.GridProc"], inject : null}, setzoom : { name : ["setzoom"], type : ["player.controller.Setzoom"], inject : null}, gridLoader : { name : ["gridLoader"], type : ["scorx.data.GridLoader"], inject : null}, pagesLoader : { name : ["pagesLoader"], type : ["scorx.data.PagesLoader"], inject : null}}};
sx.ScorxColors.ScorxYellow = 16763174;
sx.ScorxColors.ScorxGreen = 11521792;
sx.ScorxColors.ScorxGreenDark = 10139136;
sx.ScorxColors.ScorxBlue = 6737407;
sx.ScorxColors.ScorxRed = 15214407;
sx.ScorxColors.ScorxRedLight = 15818102;
